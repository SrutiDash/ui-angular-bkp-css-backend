...
// app.get('/user-details', async (req, res) => {
//   try {
//     const { user_name, parent_name, domain_name, category_name, queried_user_name, queried_parent_name, queried_domain_name, queried_category_name } = req.query;
...
//           // Check and log changes for each filter parameter
//           for (const [filterKey, queriedValue] of Object.entries(queriedFilters)) {
//             const key = filterKey.replace('queried_', '');
//             const new_value = filters[key];
//             if (queriedValue && new_value && queriedValue !== new_value) {
//               const logQuery = `
//                 INSERT INTO azamp_target.change_log (change_date, change_column, old_value, new_value, user_dimid)
//                 VALUES (CURRENT_TIMESTAMP, $1, $2, $3, $4);
//               `;
//               const logValues = [key, queriedValue, new_value, user_dimid];
//               console.log('Executing log query:', logQuery);
//               console.log('Log query values:', logValues);
//               await client.query(logQuery, logValues);
//               console.log('Log entry inserted for user_dimid:', user_dimid);
//             }
//           }
//         }
...
//   try {
//     const { 
//       user_name, parent_name, domain_name, category_name, 
//       queried_user_name, queried_parent_name, queried_domain_name, queried_category_name 
//     } = req.query;
...
//             `;
//             const oldValueResult = await client.query(oldValueQuery, [key]);
//             console.log(`oldValueResult: ${JSON.stringify(oldValueResult)}`);
//             const old_value = oldValueResult.rows.length ? oldValueResult.rows[0].new_value : null;
...
// // Endpoint to fetch user details and store user_dimid and update old_value
// app.get('/user-details', async (req, res) => {
//   try {
//     const { user_name, parent_name, domain_name, category_name } = req.query;
//     let query = 'SELECT user_dimid, user_name, parent_name, domain_name, category_name FROM dim_user WHERE 1=1';
...
//     if (result.rows.length > 0) {
//       // Log changes into change_log table and update old_value
//       const client = await pool.connect();
//       try {
//         await client.query('BEGIN');
...
//               // Insert a new entry into change_log with current timestamp, column name, old value, and the new value
//               const logQuery = `
//                 INSERT INTO change_log (change_date, change_column, old_value, new_value, user_dimid)
//                 VALUES (CURRENT_TIMESTAMP, $1, $2, $3, $4);
//               `;
...
//               // Update the old_value for the next fetch
//               const updateOldValueQuery = `
//                 UPDATE change_log
//                 SET old_value = (
//                   SELECT new_value
//                   FROM change_log AS sub
//                   WHERE sub.change_column = $1 AND sub.user_dimid = change_log.user_dimid
//                   ORDER BY change_date DESC
//                   LIMIT 1 OFFSET 1
//                 )
//                 WHERE change_column = $1 AND user_dimid = $2;
//               `;
//               await client.query(updateOldValueQuery, [key, row.user_dimid]);
//             }
//           }
//         }
...
const logChanges = async (change_column, old_value, new_value, user_dimid) => {
  const query = `
    INSERT INTO change_log (change_date, change_column, old_value, new_value, user_dimid)
    VALUES (CURRENT_TIMESTAMP, $1, $2, $3, $4)
  `;
  await pool.query(query, [change_column, old_value, new_value, user_dimid]);
};
...
//     let query =
//       "SELECT user_dimid, user_name, parent_name, domain_name, category_name FROM dim_user WHERE 1=1";
//     const queryParams = [];
...
//               // Get the latest entry from change_log for the specific user_dimid and change_column
//               console.log(`key: ${key}`);
//               console.log(`user_dimid: ${user_dimid}`);
//               const oldValueQuery = `
//                 SELECT new_value
//                 FROM change_log
//                 WHERE change_column = $1
...
//               const oldValueResult = await client.query(oldValueQuery, [key]);
//               console.log(`oldValueResult: ${JSON.stringify(oldValueResult)}`);
//               const old_value = oldValueResult.rows.length
//                 ? oldValueResult.rows[0].new_value
//                 : null;
//               if (count === 0) {
//                 constantOldValue = old_value;
//               }
//               console.log(`constantOldValue: ${constantOldValue}`);
//               console.log(`filters[key]: ${filters[key]}`);
//               console.log(`oldvalue: ${old_value}`);

//               // Update old value only if the current filter value is different from the latest new_value
//               if (
//                 filters[key] !== constantOldValue ||
//                 oldValueResult.rows[0].user_dimid != user_dimid
//               ) {
//                 const logQuery = `
//                   INSERT INTO change_log (change_date, change_column, old_value, new_value, user_dimid)
//                   VALUES (CURRENT_TIMESTAMP, $1, $2, $3, $4);
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      try {
          ...
          for (const key of Object.keys(filters)) {
            if (filters[key]) {
              ...
              if (
                ...
                `;
                const logValues = [key, old_value, filters[key], user_dimid];
                logQueries.push({ query: logQuery, values: logValues });
              }
            }
            filterCount++;
          }
        ...
        for (const loqQueryData of logQueries) {
          await client.query(loqQueryData.query, loqQueryData.values);
        }
